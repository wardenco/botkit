...
function Slackbot(configuration) {
    ...
    slack_botkit.configureSlackApp = function(slack_app_config, cb) {
        ...
        if (!slack_app_config || !slack_app_config.clientId ||
            ...
            slack_botkit.config.clientSigningSecret = slack_app_config.clientSigningSecret;
    ...
    slack_botkit.handleWebhookPayload = function(req, res) {
        ...
        if (slack_botkit.config.clientVerificationToken && payload.token !== slack_botkit.config.clientVerificationToken) {
            slack_botkit.debug('Token verification failed, Ignoring message');
            res.status(401);
            return;
        }
    ...
    slack_botkit.middleware.normalize.use(function handleEventsAPI(bot, message, next) {
        ...
        if (message.type == 'event_callback') {
            ...
            if (bot.identity == undefined || bot.identity.id == null) {
                ...
                return;
            } else if (bot.identity.id === message.user && message.subtype !== 'channel_join' && message.subtype !== 'group_join' && message.subtype !== 'app_conversation_join') {
